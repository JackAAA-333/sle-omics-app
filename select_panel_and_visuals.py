...
def main(top_n=10):
    ...
    plt.title('面板系数跨折分布 / Panel Coefficients Across CV Folds')
    plt.tight_layout()
    ...
    plt.ylabel('AUC / 曲线下面积')
    plt.title('交叉验证AUC分布 / Cross-Validated AUCs')
    plt.tight_layout()
    ...
    plt.plot(mean_fpr, mean_tpr, label=f'平均AUC / Mean AUC={mean_auc:.3f}')
    plt.plot([0,1],[0,1],'--', color='gray')
    plt.xlabel('假阳性率 / FPR'); plt.ylabel('真阳性率 / TPR'); plt.title('平均ROC曲线 / Mean ROC')
    plt.legend()
    plt.tight_layout()
